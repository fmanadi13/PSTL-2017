class Html():
    ...
    def tocToHTML(toc):
        def tocToHTMLFunction(tocList):
            html = ""
            for i in range(0, len(tocList)):
                tocItem = tocList[i]
                sectionNumberText = tocItem.headerNode.sectionNumberText
                sectionNumber = self.documentOptions.num and self.inlineTag("span", sectionNumberText, {"class": "section-number"}) or ""
                header = self.getNodeTextContent(tocItem.headerNode)
                headerLink = self.inlineTag("a", sectionNumber + header, {href: "#header-" + sectionNumberText.replace('\.g', "-")})
                subList = tocItem.childTocs.length and tocToHTMLFunction.call(self, tocItem.childTocs) or ""
                html += self.tag("li", headerLink + subList)
            return self.tag("ul", html)
        
        return tocToHTMLFunction(self, toc)

    def computeAuxDataForNode(node):
    ...
    def escapeSpecialChars(text, insideCodeElement):
        if not self.replaceRegexp:
            self.replaceRegexp = re.compile("|".join(replaceMap.keys()))
    ...
    # @implement
    def postProcess(node, currentText, insideCodeElement):
        if self.exportOptions.exportFromLineNumber and isinstance(node.fromLineNumber, "number"):
